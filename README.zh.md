...
**agent 会记住一切吗？**
便签有 2200 字符预算。`memory-auto` 会在对话后自动提炼持久事实；当添加会超出预算时，旧便签自动归档进匹配的主题文件（按关键词路由、钉住的绝不碰）而不是被丢弃。经验库无预算，放你整理的主题文档，归档内容在那里仍可检索。

**归档的技能会怎样？**
移到 `~/.dsh/skills/_archived/<name>/`，不再加载进技能列表，随时 `dsh-mind restore <name>` 恢复。

**会自动提取记忆 / 注入提示词吗？**
部分会，且现在能自我维护：`memory-auto` 在对话后自动提炼持久事实，钉住的便签每轮注入提示词（`memory-inject`）。提取受预算约束——便签将超出 2200 字符预算时，旧便签自动归档进主题文件（0.2.0 起），不会静默丢弃。设计仍保持轻量：无服务、文件存储，归档内容可语义检索。
