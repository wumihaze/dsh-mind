...
- [x] 4.1 DSH Web 插件骨架：`client/` 目录，slot 注册，主题适配
- [x] 4.2 **记忆面板**：列表、新增、删除（预算条可视化）
- [x] 4.3 **技能仪表盘**：使用频率表（按使用量排序）、来源标签
- [x] 4.4 **治理控制台**：当前状态、暂停/恢复按钮、手动触发、阈值显示
- [x] 4.5 **快照管理**：列表、时间戳、回滚按钮、清理按钮
- [x] 4.6 **设置面板**：状态概览 + 操作入口
- [x] 4.7 i18n：EN + ZH 双语
- [x] 4.8 深色/浅色主题：跟随 DSH 主题（`--dsw-*` 语义 token）
- [x] 4.9 响应式：480px 断点适配
- [x] 4.10 交互细节：加载态、空态、错误态、toast 反馈

## Phase 5：测试与质量

- [x] 5.1 集成测试：安装 → 启动 → 所有服务可用 → 工具可调用
- [x] 5.2 E2E 流程测试：记记忆 → 查记忆 → 触发治理 → 归档 → 恢复 → 回滚
- [x] 5.3 数据持久化测试：重启 DSH 后数据不丢
- [x] 5.4 Windows 兼容：路径、换行符、权限
- [x] 5.5 边界情况：记忆文件为空、技能列表为空、并发写入
- [x] 5.6 性能：1000 条记忆搜索 < 500ms

## Phase 6：文档

- [x] 6.1 README.md（EN）：功能、安装、使用、配置、FAQ
- [x] 6.2 README.zh.md：中文完整版
- [x] 6.3 架构图：一张图说清记忆层 + 技能层 + 治理层
- [x] 6.4 快速开始：3 步（装 → 用 → 看效果）
- [x] 6.5 配置参考：所有参数表格
- [x] 6.6 FAQ："数据存哪？""能卸载吗？""和 Letta 有什么区别？"
- [x] 6.7 CHANGELOG.md：版本记录
